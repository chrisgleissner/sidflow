...
The WASM build of libsidplayfp lives in its own repository,
[`chrisgleissner/libsidplayfp-wasm`](https://github.com/chrisgleissner/libsidplayfp-wasm),
and is consumed here from npm as `libsidplayfp-wasm`. Everything that used to be
described in this section — the dual-engine build, the upstream pin, engine-identity
checks, native differential parity, the golden fixtures, and the release qualification —
is that project's responsibility now, and its `AGENTS.md` documents it.

What remains true here:

- **Treat valid-looking PCM as insufficient evidence.** A published artifact was silently
  the wrong emulation for months while every test in this repository passed, because
  plausible audio is not evidence of correct audio. Anything in SIDFlow that judges
  engine output must compare against a reference, not against expectations.
- **Pin the engine, do not float it.** `libsidplayfp-wasm` is depended on by an exact
  minor range. A bump is a data-affecting change: renders produced by different engine
  versions are not interchangeable, and `sid_engine` in the export manifest records
  which one produced a corpus.
- **Report engine defects upstream**, at the libsidplayfp-wasm repository, with the
  failing SID and the `getTuneMd5()` key. Do not work around them here.
- **C64 ROMs are a prerequisite for correct playback, not an RSID-only unlock.** Without
  KERNAL, BASIC and CHARGEN a tune initialises and then never advances, which reads as a
  silent render rather than as an error. `scripts/fetch-roms.ts` puts them in
  `workspace/roms`, and the render path loads them before anything else. The measurement
  this comes from — the engine against a real C64 Ultimate, captured off its multicast
  audio mirror — lives in the C64 Commander repository at
