...
- [ ] Expand preferences schema (theme, ROM set, training toggle, iteration budget, sync cadence) and persist to `localStorage` + IndexedDB with migration/versioning tests.
...
- [ ] Execute load tests simulating ≥5k concurrent sessions (SAB + HLS mix), validate CDN/offload strategy, and capture resulting CPU/memory utilization.
