...
> **Where to work.** `sidflow` lives at `/mnt/data/dev/c64/sidflow` (`/home/chris/dev` is a symlink to
> `/mnt/data/dev`, so the two spellings are the same directory). `1e59ab6` is `origin/main`, so the
> 0.7.0 tree is already the base — **branch off `main`** for this work, e.g.
> `feat/export-audit-0.8.0`. Do not build on `fix/libsidplayfp-wasm-pin-and-residfp-audit` (merged) or
> on whatever branch the checkout happens to be sitting on. Part D is a separate repository with its
> own checkout and its own branch; see that section.
...
> `SID_RADIO_STYLE_TILES` (`useSidRadio.ts:389-400`), as D1 describes. So **corrected masks do reach
...
**Delegate this to a subagent running inside the `c64commander` checkout**, not from `sidflow`. That
...
- **Working directory:** `/home/chris/dev/c64/c64commander` — the repository's only checkout (`git
  worktree list` shows just this one, reported under its resolved path). The `feat/sid-radio` worktree
  this task previously named, `/home/chris/dev/c64/c64commander-sid-radio`, **no longer exists**; do
  not try to use it. Note that `/home/chris/dev` is a symlink to `/mnt/data/dev`, so
  `/mnt/data/dev/c64/c64commander` is the same directory under its resolved name — use the
  `/home/chris/…` form above.
- **Branch:** `feat/sid-radio` **was merged to `main`** as `df3c3e31` (PR #320, 2026-07-27 18:06 +01:00)
  and the local branch is gone; the checkout sits on `main`, in sync with `origin/main`. So **create a
  new branch off `main`** — `feat/sid-radio-station-populations` — and commit onto that. Do not
  reopen or re-push `feat/sid-radio`.
...
> **Source references.** Every `c64commander` file and line number in Part D is `df3c3e31`
> (`origin/main` as of 2026-07-27), the merge of `feat/sid-radio`. They were re-verified against that
> tree, not carried over from the pre-merge branch.

### D1. Guard against zero-population styles

`SID_RADIO_STYLE_TILES` (`src/pages/playFiles/hooks/useSidRadio.ts:389-400`) hardcodes all nine
persona tiles with client-side labels, and `SidRadioLauncherSheet.tsx:63` renders them straight from
that array. Station membership is `bundle.styleMask[ordinal] & (1 << bit)`
...
**Only after `sidflow-data` `0.8.0` is published** (Part C). Update `SIDCORR_RELEASE_TAG` and
`SIDCORR_BUNDLE_SHA256` in `src/lib/sidRadio/sidcorrRelease.ts` — today they read
`"sidcorr-hvsc-full-20260726T203707Z"` and `081664d8…cba7c5`. Note the comment there: *"the build
script keeps its own `SIDCORR_RELEASE` copy and a test asserts the two never drift"* — that copy is
`scripts/fetch-sidcorr.mjs:34`, and the test is `tests/unit/scripts/fetchSidcorr.test.ts`. Update both
constants and let that test verify it.

`SIDCORR_EXPECTED` in the same file pins `fileCount: 61157`, `trackCount: 87868`,
`neighborsPerTrack: 3` and `styleCount: 9` for the `SIDCORR_REAL=1` golden test. 0.8.0 changes none of
those (the tiny rebuild touches only the style-mask section and the manifest, per B3), so they should
stay as they are — but re-check them against the published 0.8.0 manifest rather than assuming.

Also fix the stale reference in `docs/plans/sid-station/spec.md:190` (§2.1 "What the current release
contains"), which still cites release `sidcorr-hvsc-full-20260407T115218Z` — along with the
`file_count` 60,571 / `track_count` 87,073 table that follows it — while the code is pinned to the
0.7.0 release. That is the 0.5-era bundle whose ratings were 94% identical; the table is a materially
misleading citation, not just a stale tag.
...
Regenerate with `npm run screenshots` — `scripts/run-screenshots-with-prune.mjs`, which runs
`screenshots:raw` (`playwright test --grep @screenshots --workers 1`) and then
`screenshots:prune-identical` (`scripts/revert-identical-pngs.mjs`) to revert unchanged output. Scope
the run as narrowly as the harness allows.
...
Because `feat/sid-radio` is already merged, this is a fresh branch off `main` rather than a
continuation. If D3 lands materially later than D1/D2, split it: open the guard work as its own PR
from `feat/sid-radio-station-populations` and branch the re-pin separately once `sidflow-data` `0.8.0`
exists, rather than parking a green branch waiting on Part C.
...
- The work is on a new branch off `main` (`feat/sid-radio-station-populations`), not on the merged
  `feat/sid-radio`, with `AGENTS.md`'s required reporting in the summary.
...
- [ ] A new `c64commander` branch off `main` (`feat/sid-radio-station-populations` — `feat/sid-radio`
      itself was merged as `df3c3e31`) carries the zero-population guard, the tile population counts,
      the 0.8.0 re-pin, and a minimal, enumerated screenshot refresh (Part D).
