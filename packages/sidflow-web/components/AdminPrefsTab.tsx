...
export function AdminPrefsTab({ onStatusChange }: AdminPrefsTabProps) {
    ...
    async (key: 'kernal' | 'basic' | 'chargen', rawValue: string | null) => {
      ...
      try {
        ...
        if (response.success) {
          ...
          const label = key === 'kernal' ? 'KERNAL' : key === 'basic' ? 'BASIC' : 'CHARGEN';
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="space-y-4 text-sm">
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-col gap-2 md:flex-row">
              ...
              />
              <Button variant="secondary" onClick={handleCustomPathSave} disabled={isSavingCollection}>
                Save
              </Button>
              <Button variant="ghost" onClick={handleResetCollection} disabled={isSavingCollection}>
                Reset
              </Button>
            </div>
            <p className="text-[11px] text-muted-foreground">
              The path must exist on disk. Relative paths resolve against the repository root.
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="c64-border">
        ...
        <CardContent className="grid gap-4 text-sm">
          <fieldset className="grid gap-2">
            <legend className="text-xs font-semibold text-muted-foreground">KERNAL ROM</legend>
            <div className="flex flex-col gap-2 md:flex-row">
              <Input value={kernalPath} onChange={(event) => setKernalPath(event.target.value)} />
              <div className="flex gap-2">
                <Input
                  ...
                  className="hidden"
                />
                <Button variant="outline" onClick={() => handleBrowseFile('kernal')}>
                  <FolderOpen className="mr-2 h-4 w-4" /> Browse
                </Button>
                <Button variant="secondary" onClick={() => saveRomPath('kernal', kernalPath)} disabled={isSavingRom}>
                  Save
                </Button>
              </div>
            </div>
          ...
          <fieldset className="grid gap-2">
            <legend className="text-xs font-semibold text-muted-foreground">BASIC ROM</legend>
            <div className="flex flex-col gap-2 md:flex-row">
              <Input value={basicPath} onChange={(event) => setBasicPath(event.target.value)} />
              <div className="flex gap-2">
                <Input
                  ...
                  className="hidden"
                />
                <Button variant="outline" onClick={() => handleBrowseFile('basic')}>
                  <FolderOpen className="mr-2 h-4 w-4" /> Browse
                </Button>
                <Button variant="secondary" onClick={() => saveRomPath('basic', basicPath)} disabled={isSavingRom}>
                  Save
                </Button>
              </div>
            </div>
          ...
          <fieldset className="grid gap-2">
            <legend className="text-xs font-semibold text-muted-foreground">CHARGEN ROM</legend>
            <div className="flex flex-col gap-2 md:flex-row">
              <Input value={chargenPath} onChange={(event) => setChargenPath(event.target.value)} />
              <div className="flex gap-2">
                <Input
                  ...
                  className="hidden"
                />
                <Button variant="outline" onClick={() => handleBrowseFile('chargen')}>
                  <FolderOpen className="mr-2 h-4 w-4" /> Browse
                </Button>
                <Button
                  variant="secondary"
      ...
      <Card className="c64-border">
        <CardHeader>
          <CardTitle className="petscii-text text-accent">THEME & FONT</CardTitle>
          <CardDescription className="text-muted-foreground">
