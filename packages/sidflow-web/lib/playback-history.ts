...
export function addToPlaybackHistory(entry: Omit<PlaybackHistoryEntry, 'timestamp'>): void {
  ...
  try {
    ...
    const payload = trimmed as unknown as JsonValue;
    
    localStorage.setItem(HISTORY_KEY, stringifyDeterministic(payload));
  } catch (error) {
...
export function removeFromHistory(sidPath: string): void {
  ...
  try {
    ...
    const payload = filtered as unknown as JsonValue;
    localStorage.setItem(HISTORY_KEY, stringifyDeterministic(payload));
  } catch (error) {
