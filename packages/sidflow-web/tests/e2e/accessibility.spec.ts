...
async function gotoWithRetry(page: Page, url: string, maxRetries = 3): Promise<void> {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            await Promise.race([
                page.waitForSelector('[role="tabpanel"]', { state: 'visible', timeout: 30_000 }),
                page.waitForSelector('main', { state: 'visible', timeout: 30_000 }),
                page.waitForSelector('h1, h2, h3', { state: 'visible', timeout: 30_000 }),
            ]).catch(() => {});
...
test.describe('Accessibility Audit', () => {
    ...
    test.describe('Focus Management', () => {
        ...
        test('should trap focus in modal dialogs', async ({ page }) => {
            ...
            if (!loginVisible) {
                console.log('[A11y] Login button not visible - skipping focus trap test');
                test.skip();
                return;
            }
